...
- **mixWarnings** — (optional) continuous mix-health warnings derived from current analyser state and spectral slot assignments. Entries include a `type`, `severity` (0.0–1.0), human-readable `message`, and contextual fields such as `trackId`, `trackIds`, `band`, `peak`, `headroom`, or `crest`.
