  ...
  parameters: {
    ...
    properties: {
      ...
      paramShapes: {
        type: 'object',
        description:
          'Inline parameter shapes — describe parameter changes over the pattern duration as functions. ' +
          'Each key is a controlId, each value is a shape definition. Shapes expand to per-step ParameterEvent p-locks. ' +
          'Simpler than adding a Tides modulator — use for pattern-scoped parameter motion. ' +
          'Shape types: "ramp_up", "ramp_down" (period + range), "triangle" (period + range), ' +
          '"sine" (period + range + optional phase), "square" (period + range), ' +
...
const setTransportTool: ToolSchema = {
  name: 'set_transport',
  description:
    'Change tempo, swing, time signature, transport mode (pattern/song), or start/stop playback. Takes effect after this response.',
  parameters: {
    type: 'object',
    properties: {
      bpm: {
      ...
      playing: {
        type: 'boolean',
        description: 'Start (true) or stop (false) playback.',
      },
      ...
      modules: {
        ...
        items: {
          ...
          properties: {
            type: {
              type: 'string',
              enum: ['knob-group', 'macro-knob', 'xy-pad', 'step-grid', 'chain-strip', 'piano-roll', 'level-meter', 'pad-grid'],
