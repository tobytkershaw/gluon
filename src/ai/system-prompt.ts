...
- **Surface & metadata**: \`set_surface\` composes a track's UI surface from modules (knob-group, macro-knob, xy-pad, step-grid, chain-strip). \`pin_control\` pins raw controls as knob-group modules. \`set_track_meta\` sets name, claimed, importance, musicalRole. \`inspect_chain\` introspects signal chains (mode: "explain" for description, "simplify" for redundancy detection).
...
- **\`move\`**: single-parameter tweaks (one knob turn). Use for tweaking one timbre control, nudging decay. For track volume/pan, use \`set_track_meta\`.
