...
function tidesControls(): ControlSchema[] {
  return [
    ...
    makeTidesControl(
      ...
      'Smoothness',
      'brightness',
      'Moves from smoothing and rounding on one side of center to wavefolding, kinks, and added harmonics on the other.',
      0.5,
    ),
    ...
    ),
    {
      ...
      semanticRole: 'body',
      description: 'Output signal family: gates/clock pulses (0), amplitude contour (1), slope or phase relationships (2), or frequency/ratio output (3).',
      readable: true,
      writable: true,
    ...
    } as ControlSchema,
    {
      ...
      kind: 'discrete' as ControlKind,
      semanticRole: 'pitch',
      description: 'Operating range. Gluon currently exposes two coarse ranges: control rate (0) for LFO/envelope behavior and audio rate (1) for oscillator behavior. The hardware has an additional middle range that is not yet modeled.',
      readable: true,
      writable: true,
