...
describe('ModelStatusIndicator', () => {
  it('shows "Connected" when both are available', () => {
  ...
  it('shows "Connected (no audio eval)" when listener is unavailable', () => {
    render(<ModelStatusIndicator plannerConfigured={true} listenerConfigured={false} />);
    expect(screen.getByTestId('model-status-label').textContent).toBe('Connected (no audio eval)');
  });

  it('shows "Disconnected" when both are unavailable', () => {
    render(<ModelStatusIndicator plannerConfigured={false} listenerConfigured={false} />);
    expect(screen.getByTestId('model-status-label').textContent).toBe('Disconnected');
  });

  it('uses teal dot when both available', () => {
    render(<ModelStatusIndicator plannerConfigured={true} listenerConfigured={true} />);
    const dot = screen.getByTestId('model-status-dot');
    expect(dot.className).toContain('bg-teal-400');
  ...
  it('uses teal dot with amber ring when only listener unavailable', () => {
    render(<ModelStatusIndicator plannerConfigured={true} listenerConfigured={false} />);
    const dot = screen.getByTestId('model-status-dot');
    expect(dot.className).toContain('bg-teal-400');
    // Amber ring applied via inline boxShadow style
    expect(dot.style.boxShadow).toContain('fbbf24');
  });

  it('renders compact mode (dot only, no label)', () => {
    render(<ModelStatusIndicator plannerConfigured={true} listenerConfigured={true} compact />);
    const dot = screen.getByTestId('model-status-dot');
    expect(dot).toBeTruthy();
    expect(screen.queryByTestId('model-status-label')).toBeNull();
  });
});
