...
#
# Shared helpers are sourced from qa-playwright-smoke.sh — this script
# depends on the same infrastructure (run_pw, eval_js, submit_ai_prompt, etc.).
...
# ---------------------------------------------------------------------------
# Helpers — copied from qa-playwright-smoke.sh for standalone operation
# ---------------------------------------------------------------------------
...
submit_ai_prompt() {
  ...
  max_attempts=$(( AI_SETTLE_TIMEOUT / 2 ))
...
el => new Promise((resolve) => {
  ...
  req.onsuccess = () => {
    ...
    getAll.onsuccess = () => {
      ...
      // Return the most recently updated project's session
...
scenario_s7_merge_regression() {
  ...
  # Tool count check — verify GLUON_TOOLS array has expected count
  local tool_count
...
PY
  ...
  if [[ "$tool_count" -ge 20 && "$tool_count" -le 30 ]]; then
    record_result "s7_tool_count" "pass" "GLUON_TOOLS has $tool_count tools (within 20-30 range)."
  else
    record_result "s7_tool_count" "fail" "GLUON_TOOLS has $tool_count tools — expected 20-30."
  fi
}
...
# Build the project and measure the system prompt via a simple token estimate.
# We count tokens as roughly words * 1.3 (conservative for English + code).
# Read the system-prompt.ts source and estimate the template size.
with open('src/ai/system-prompt.ts', 'r') as f:
    content = f.read()

# Extract the buildSystemPrompt function's template literal content
...
for field in required_fields:
    if field not in content:
        missing.append(f'CompressedState.{field}')

for field in required_track_fields:
    if field not in content:
        missing.append(f'CompressedTrack.{field}')
...
missing = [t for t in m6_tools if f"name: '{t}'" not in content]
if missing:
    print('missing:' + ','.join(missing))
else:
    print('ok')
