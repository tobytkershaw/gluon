...
describe('Pattern Undo', () => {
  ...
  it('RegionSnapshot revert restores events and re-projects pattern', () => {
    ...
    expect(getTrack(afterSketch, 'v0').stepGrid.steps[0].gate).toBe(true);
    expect(getTrack(afterSketch, 'v0').patterns[0].events.length).toBe(2);

    // Undo
    const undone = applyUndo(afterSketch);
    expect(getTrack(undone, 'v0').stepGrid.steps[0].gate).toBe(false);
    expect(getTrack(undone, 'v0').patterns[0].events.length).toBe(0);
  });
  ...
  it('old PatternSnapshot entries still revert correctly', () => {
    ...
    expect(getTrack(session, 'v0').stepGrid.steps[0].gate).toBe(true);
  ...
  it('mixed undo stack: PatternSnapshot followed by RegionSnapshot', () => {
    ...
    expect(session.undoStack[0].kind).toBe('pattern');
    expect(session.undoStack[1].kind).toBe('pattern-edit');
    ...
    expect(getTrack(session, 'v0').patterns[0].events.length).toBe(1);
    expect(session.undoStack.length).toBe(1);

    // Undo PatternSnapshot (legacy)
    session = applyUndo(session);
    expect(getTrack(session, 'v0').stepGrid.steps[0].gate).toBe(false);
  ...
  it('AI sketch after human edit: human edit persists through undo', () => {
    ...
    const undone = applyUndo(session);
    expect(getTrack(undone, 'v0').stepGrid.steps[0].gate).toBe(false);
    expect(getTrack(undone, 'v0').patterns[0].events).toHaveLength(0);
  });
