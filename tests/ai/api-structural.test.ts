...
function minimalArgsForTool(toolName: string): Record<string, unknown> {
  switch (toolName) {
    ...
    case 'setup_return_bus':
      return { sourceTrackId: 'v0', processorType: 'clouds', description: 'test return bus' };
    case 'set_master':
      return { volume: 0.7 };
    case 'manage_pattern':
    ...
    case 'relate':
      return { sourceTrackId: 'v0', targetTrackId: 'v1', relation: 'align', description: 'align bass to kick' };
    case 'manage_motif':
      return { action: 'list' };
    case 'save_patch':
...
describe('API Structural Integrity', () => {
  ...
  it('projectAction handles all action types that executeFunctionCall can produce', () => {
    ...
    const analysisOnlyTools = ['listen', 'render', 'analyze', 'inspect_chain', 'suggest_reactions', 'save_patch', 'list_patches', 'recall_memories'];
  ...
  it('tool count matches expected value', () => {
    ...
    expect(GLUON_TOOLS.length).toBe(49);
  });
